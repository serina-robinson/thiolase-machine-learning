...
# Read in the activity levels
act <- read_csv("data/machine_learning/20191228_enzyme_activity_level.csv") %>%
  dplyr::mutate(id = paste0(org, "_", cmpnd)))
act$org

dat$id[grep("Xanthomonas")]
...
dat <- rawdat %>%
  dplyr::mutate(id = paste0(word(org, 2, sep = " "), "_", substrate)) 

act$id[!act$id %in% dat$id]
dat$id
#%>%
