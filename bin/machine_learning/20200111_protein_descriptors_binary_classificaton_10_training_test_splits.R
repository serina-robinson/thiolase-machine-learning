...
rf_20200111$bestTune$mtry
rf_20200111$bestTune$splitrule
...
for(i in 1:10) {
  ...
  rf_1 <- ranger(y_train ~., data = form_train, num.trees = 1000, 
                 splitrule = rf_20200111$bestTune$splitrule,
                 mtry = rf_20200111$bestTune$mtry, 
                 min.node.size = rf_20200111$bestTune$min.node.size,
...
pdf("output/machine_learning/20200111_rf_classification_10iterations_varimp_avged.pdf", height = 7, width = 5)
vpavg
dev.off()
...
regression <- c("151", "173", "172", "321", "313", "316", "284", "240", "169", "318")

intersct <- intersect(regression, imps)
paste0(intersct, collapse = "+")

diff <- setdiff(regression, imps)
