...
viridis(10, option = "inferno")

my.formula = y ~ x
pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_dimethyl_gray_colored.pdf", width = 7, height = 6)
ggplot(data = combdat, aes(x = area_sa, y = avg_activity)) +
  geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(color = "#4B0C6BFF", aes(
                 size = volume_sa), alpha = 0.15) +
  geom_point(data = subset(combdat, grepl("atraur|Halobacter|paraconglomeratum", org)), 
             color = "#4B0C6BFF",
             aes(size = volume_sa), alpha = 1) +
  theme_pubr() +
  ...
  ggrepel::geom_text_repel(data = subset(combdat, grepl("atraur|Halobacter|paraconglomeratum", org)), segment.size = 0.2,
                           # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
                           segment.color = "grey50", #direction = "x",
                           text.color = "#781C6DFF",
                           aes(label = spec), fontface = "italic") +
...
comb2$area_sa <- as.numeric(comb$area_sa)
comb2$volume_sa <- as.numeric(comb$volume_sa)
...
pl1 <- ggplot(data = combdat2, aes(x = area_sa, y = avg_activity)) +
  ...
  theme(axis.ticks.x = element_blank(), axis.text.x = element_blank(), axis.title.x = element_blank(),
        axis.line.x.bottom = element_blank(), legend.title = element_blank()) +
 # scale_color_viridis(option = "plasma") +
  # ggrepel::geom_text_repel(data = subset(combdat, avg_activity > 1.85 | area_sa > 580 | area_sa < 275), segment.size = 0.2,
  #                          # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
  #                          segment.color = "grey50", #direction = "x",
...
pl2 <- ggplot(data = combdat, aes(x = area_sa, y = avg_activity)) +
  ...
  geom_point(data = combdat, aes(x = area_sa, y = avg_activity, size = volume_sa),
             color = "#008080", 
             alpha = ifelse(grepl("atraur|Halobacter|paraconglomeratum", combdat$org), 1, 0.15)) +
  theme_pubr(base_size = 16) +
...
pl2
dev.off()
min(combdat$volume_sa)
max(combdat$volume_sa)
maxx()pdf("output/machine_learning/horiz_stacked_volume_plot.pdf", height = 9, width = 6)
plot_grid(pl1, pl2, ncol = 1)
dev.off()
#### EVERYTHING

pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_tert_carbon_R2.pdf", width = 8, height = 8)
ggplot(data = combdat, aes(x = area_sa, y = volume_sa)) +
  geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(aes(color = avg_activity,
                 size = avg_activity), alpha = 0.7) +
  theme_pubr() +
  scale_color_viridis(option = "plasma") +
  ggrepel::geom_text_repel(data = subset(combdat, avg_activity > 1.25 | area_sa > 550), segment.size = 0.2,
                           # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
                           segment.color = "grey50", #direction = "x",
...
pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_tert_carbon_pocket_volume_R2.pdf", width = 8, height = 8)
ggplot(data = combdat, aes(x = area_sa, y = volume_sa)) +
  geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(aes(color = avg_activity,
                 size = volume_sa), alpha = 0.7) +
  theme_pubr() +
  scale_color_viridis(option = "plasma") +
  ggrepel::geom_text_repel(data = subset(combdat, avg_activity > 1.25 | area_sa > 550), segment.size = 0.2,
                           # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
...
pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_tert_carbon.pdf", width = 8, height = 8)
ggplot(data = combdat, aes(x = area_sa, y = avg_activity)) +
  geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(aes(color = avg_activity,
                 size = volume_sa), alpha = 0.7) +
  theme_pubr() +
  scale_color_viridis(option = "plasma") +
  ggrepel::geom_text_repel(data = subset(combdat, avg_activity > 1.25 | area_sa > 600 | area_sa < 275), segment.size = 0.2,
                          # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
                           segment.color = "grey50", #direction = "x",
                           aes(label = org), fontface = "italic") +
  ylab("Enzyme activity") +
...
dimethyl <- comb %>%
  ...
  dplyr::select(org, area_sa, activity, avg_activity, volume_sa) %>%
  # dplyr::mutate(num_sub = paste0(rep(1:14, each = n), substrate)) %>%
  distinct() 
dev.off()
head(dimethyl)
pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_tert_carbon_dimethyl.pdf", width = 8, height = 8)
ggplot(data = dimethyl, aes(x = area_sa, y = activity)) +
  #geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(aes(color = activity,
                 size = volume_sa), alpha = 0.7) +
  theme_pubr() +
  scale_color_viridis(option = "plasma") +
  ggrepel::geom_text_repel(data = subset(dimethyl, activity > 1.25 | area_sa > 550), segment.size = 0.2,
                           # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
                           segment.color = "grey50", #direction = "x",
                           aes(label = org), fontface = "italic") +
  ylab("Enzyme activity") +
...
pdf("output/machine_learning/solvent_pocket_volume_surface_area_plot_tert_carbon_TMA.pdf", width = 8, height = 8)
ggplot(data = tma, aes(x = area_sa, y = activity)) +
  #geom_smooth(method = "lm", se = FALSE, color="gray90", formula = my.formula, linetype = 2) +
  geom_point(aes(color = activity,
                 size = volume_sa), alpha = 0.7) +
  theme_pubr() +
  scale_color_viridis(option = "plasma") +
  ggrepel::geom_text_repel(data = subset(tma, activity > 1.25 | area_sa > 600), segment.size = 0.2,
                           # nudge_y = 20, #subset(combdat, avg_activity > 1.25)$volume_sa,
