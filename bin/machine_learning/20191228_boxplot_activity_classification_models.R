...
rawdat <- read_csv("data/machine_learning/20191228_1095_training_examples_pass2.csv") %>%
  dplyr::mutate(id = paste0(org, "_", substrate))
rawdat$id[grep("Pseudoxanthomonas", rawdat$id)] <- gsub("Pseudoxanthomonas NA", "Pseudoxanthomonas sp.", rawdat$id[grep("Pseudoxanthomonas", rawdat$id)])
rawdat$id

olddat <-read_csv("data/machine_learning/20191222_1095_training_examples_pass1.csv") %>%
  dplyr::mutate(id = paste0(org, "_", substrate))
rawdat$id[!rawdat$id %in% olddat$id]

          
missing <- olddat[!olddat$id %in% rawdat$id,] %>%
  dplyr::mutate(activity_level = "low")
...
nozdat <- nearZeroVar(combdat, saveMetrics = TRUE)
which_rem <- rownames(nozdat)[nozdat[,"nzv"] == TRUE]

### INCLUDING BOTH SEQUENCE AND CHEMICAL FEATURES
dat <- combdat %>%
  dplyr::mutate(is_active = as.factor(case_when(activity_level == "low" ~ "N",
                                                TRUE ~ "Y"))) %>%
  dplyr::select(-which_rem, -org, -substrate) %>%
  dplyr::select(id, contains("_"),  contains("PC"), is_active)
 #338 variables remaining with non-zero variance
...
sink("output/machine_learning/random_forest_boxplot_activity_binary_classification_results.txt")
cm_rf
sink()
...
pdf("output/machine_learning/random_forest_boxplot_activity_binary_classification_vimp.pdf", width = 6, height = 6)
ggplot(data = vimp,
...
sink("output/machine_learning/Naive_Bayes_boxplot_activity_binary_classification_results.txt")
cm_nb
sink()
...
pllist[[1]]
ggsave("output/machine_learning/nb_boxplot_activity_boxplot_activity_binary_classification_matrix_20191228.jpeg", height=7, width=7, units='in')
...
sink("output/machine_learning/Neural_network_boxplot_activity_binary_classification_results.txt")
cm_nnet
sink()
...
pllist[[1]]
ggsave("output/machine_learning/nnet_boxplot_activity_binary_classification_matrix_20191228.jpeg", height=7, width=7, units='in')
...
pdf("output/machine_learning/svm_radial_feat_select_var_imp_boxplot_activity_binary_classification.pdf", width = 6, height = 6)
ggplot(data = vimp, 
       aes(x=reorder(variable,importance), y=importance, fill=importance))+ 
  geom_bar(stat="identity", position="dodge")+ coord_flip()+
  ylab("Variable Importance")+
...
pllist[[1]]
ggsave("output/machine_learning/svm_radial_conf_matrices_boxplot_activity_binary_classification_20191228.jpeg", height=7, width=7, units='in')
