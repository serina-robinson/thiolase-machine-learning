...
write_csv(data.frame(tow, stringsAsFactors = F), "shiny_app/data/seqft_colnames.csv", col_names = F)
...
dat <- dedup %>%
  ...
  dplyr::mutate(is_active = as.factor(case_when(activity == 0 ~ "N",
                                                TRUE ~ "Y"))) %>%
  dplyr::select(-which_rem, -org, -substrate, -enzyme, -activity, -acc) #-IUPAC, -SMILES, -cmpnd_abbrev, ) 

# Write unscaled dataset to file
write_csv(dat, "data/machine_learning/20200323_1095_training_examples_no_prot_feats_unscaled.csv")
