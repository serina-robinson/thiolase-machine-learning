...
# Normalize all
rep1 <- fread(tmafils[1], fill = T, header = F, skip = 3, nrows = 548, data.table = F) # specific for heptynoate
tail(rep1)
rep2 <- fread(tmafils[1], fill = T, header = F, skip = 3, nrows = 548, data.table = F)
head(rep2)
tail(rep2)
