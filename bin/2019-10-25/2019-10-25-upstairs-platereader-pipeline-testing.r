...
dat4 <- dat3 %>%
  ...
  dplyr::mutate(winners = case_when(grepl(paste0(c("Kyto", "XC", "Pet", "pNP", "Gran", "Mobil"), collapse = "|"), variable) ~ variable,
                                    TRUE ~ " inactive")) %>%
  dplyr::filter(winners != " inactive")
head(dat4)
...
pdf(paste0(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_without_errorbars_normalized_test.pdf")), width = 18, height = 14)
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_linear_slopes_normalized_final_test.pdf"), width = 16, height = 14)
