...
temp <- read_excel(paste0("data/", date, "/Plate Set up SynBio Paper TMA biotin.xlsx"), skip = 3) %>%
  janitor::clean_names() %>%
...
temp[!grepl("Pet28_", temp)] <- gsub("biotin_", "", temp[!grepl("Pet28_", temp)])
temp
...
tmafils <- list.files(paste0("data/", date, "/"), pattern = "TMA", full.names = T)
...
normalize_all_biotin <- function(x) { 
  tma1 <- read_excel(x) %>%
    janitor::clean_names() %>%
  ...
  pet28 <- dat0 %>%
    dplyr::filter(grepl("biotin_Pet28", variable)) %>% # TODO change this to biotin_pet28
    group_by(time) %>%
...
res <- normalize_all_biotin(tmafils)
head(res)
