...
pacman::p_load("tidyverse", "readxl", "randomcoloR", 
               "RColorBrewer", "ggplot2", "broom", "maditr")
...
temp <- read_excel(paste0("data/", date, "/Plate Set up SynBio Paper ClPh propionate benzoate and dimethyl.xlsx"), skip = 3) %>%
  janitor::clean_names() %>%
...
temp[!grepl("Pet28_", temp)] <- gsub("benzoate_", "", temp[!grepl("Pet28_", temp)])
temp

#biot_temp <- temp[grepl("biotin_", temp)]
# biot_temp <- gsub("biotin_", "", biot_temp)
# biot_temp

# Read in the raw data 
tmafils <- list.files(paste0("data/", date, "/"), pattern = "ClPh", full.names = T)
...
normalize_all_benzoate <- function(x) { 
  tma1 <- read_excel(x) %>%
    janitor::clean_names() %>%
    dplyr::select(-temperature_c) %>%
    dplyr::select_if(grepl("time|10|11|12", colnames(.)))
  ...
  tma1
  # Select only the template files with biotin, pNP or Pet28 in the name
  ...
  dat0 <- tma1 %>%
    ...
    dplyr::filter(!variable %in% c("Pet28_1", "Pet28_2", "Pet28_3", "Nothing", "XC")) %>%
    dplyr::filter(!grepl("dimethyl_", variable))

  ## TODO: average 3 Pet28s
  pet28 <- dat0 %>%
    dplyr::filter(grepl("benzoate_Pet28", variable)) %>% # TODO change this to biotin_pet28
    group_by(time) %>%
...
res <- normalize_all_benzoate(tmafils)
head(res)
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_with_errorbars_normalized.pdf"), width = 20, height = 14)
pl <- ggplot(dat5, aes(x=time, y=mean, color=winners)) +
  geom_point() +
  labs(y = "nmol pNP", x = "Time (minutes)") +
  geom_errorbar(aes(ymax=mean + sd, ymin = mean - sd), width=0.3,size=0.6)+
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_test_assessment.pdf"),  width = 10, height = 10)
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_linear_slopes_normalized_final.pdf"), width = 18, height = 14)
