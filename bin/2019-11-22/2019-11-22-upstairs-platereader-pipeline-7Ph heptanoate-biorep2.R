...
# Normalize
rep1 <- fread(tmafils[1], fill = T, header = F, skip = 1, nrows = 548, data.table = F) 
...
pdf(paste0(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_without_errorbars_normalized.pdf")), 
    width = 20, height = 14)
...
a <- dat5 %>%
  ...
  dplyr::slice(0:30) %>% # trimmed to 30 because of a strange bump in one line
  dplyr::ungroup() %>%
