...
temp[!grepl("Pet28_", temp)] <- gsub("dimethyl_", "", temp[!grepl("Pet28_", temp)])
temp
...
normalize_all_dimethyl <- function(x) { 
  tma1 <- read_excel(x) %>%
    janitor::clean_names() %>%
  ...
  dat0 <- tma1 %>%
    ...
    dplyr::filter(!grepl("benzoate_", variable))

  ## TODO: average 3 Pet28s
  pet28 <- dat0 %>%
    dplyr::filter(grepl("dimethyl_Pet28", variable)) %>% # TODO change this to biotin_pet28
    group_by(time) %>%
...
res <- normalize_all_dimethyl(tmafils)
head(res)
