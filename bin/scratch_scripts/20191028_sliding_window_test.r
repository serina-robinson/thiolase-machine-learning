...
pacman::p_load("tidyverse", "readxl", "randomcoloR", "plyr", "RColorBrewer", "ggplot2", "zoo", "ggpubr")
...
pal2 <- c("gray80", "dodgerblue", "goldenrod", pal[c(1, 3:5, 8)], "blue", "gold1", "black")
...
#   r2 <- summ$r.squared
#   slope <- coef(m)[2]
#   #return(list(slope, r2))
#   #return(list(r2 = r2, slope = slope))
#   return(slope)
# }

# m1 <- lm(mean ~ minutes, as.data.frame(gran, stringsAsFactors = F))
# sum1 <- summary(m1)
# sum1$r.squared
# 
# co <- rollapplyr(zoo(gran), 10, f, by.column=F)
# max(co)
# 
# pl <- ggplot(gran,  aes(x = minutes, y = mean)) + 
#   geom_point() +
...
# Calculate slopes within each organism
...
summary(resll$r2)
write_csv(resll, "output/test_slope_output.csv")
