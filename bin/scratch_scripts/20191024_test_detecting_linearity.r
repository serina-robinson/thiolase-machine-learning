# Install packages
pacman::p_load("tidyverse", "readxl", "randomcoloR", "RColorBrewer", "ggplot2", "data.table", "broom")
...
# Read in datasets
# dat5 <- read_csv(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_normalized_data.csv")) %>%
#   janitor::clean_names() %>%
#   dplyr::mutate(winners = case_when(winners == "inactive" ~ " inactive",
#                 TRUE ~ winners))
...
calc_slope <- function (a) {
  m <- lm(mean ~ minutes, data = a)
  return(m)
}
head(dat5)

a <-#dat5[grepl("Granulosicoccus", dat5$variable),] %>%
  dat5 %>%
  ungroup() %>%
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "/slopes_plotted.pdf", width = 20, height = 20)
