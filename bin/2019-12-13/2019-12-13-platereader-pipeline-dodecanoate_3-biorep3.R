...
tmafils <- tmafils[!grepl("~|Plate", tmafils)] # remove any temporary files
tmafils
...
normalize_all_dodecanoate <- function(x) { 
  ...
  pet28 <- dat0 %>%
    dplyr::filter(grepl("dodecanoate_Pet28_1|dodecanoate_Pet28_2", variable)) %>% # Removed 3 which is irregular 
    group_by(time) %>%
