...
normalize_all <- function(x) { 
  ...
  pet28 <- dat0 %>%
    dplyr::filter(grepl("Pet28_1|Pet28_2", variable)) %>% # REMOVED Pet28_3 because of slope
    group_by(time) %>%
...
res <- normalize_all(tmafils)

resbind <- res %>%
  dplyr::filter(!grepl("ClPh|Nothing_", variable))
...
windowsize <- 5 # Note had to change window size to 5 for butoxy because Kytococcus was so fast
