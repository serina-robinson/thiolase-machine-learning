...
fildir <- list.files(paste0("data/", date, "/plate_templates/"), full.names = T) #Plate Set up SynBio Paper heptanoate dodecanoate.xlsx"
templ <- fildir[grep(cmpnd, fildir)]
templ
...
normalize_all_dodecanoate <- function(x) { 
  ...
  # Select only the template files with dodecanoate, pNP or Pet28 in the name
  ...
  pet28 <- dat0 %>%
    dplyr::filter(grepl("dodecanoate_Pet28_1|dodecanoate_Pet28_2", variable)) %>% # TODO change this to dodecanoate_pet28
    group_by(time) %>%
...
tmafils
res <- normalize_all_dodecanoate(tmafils)
head(res)
...
tofind <- unique(dat4$variable)[grep("Pet28", unique(dat4$variable))]
tofind
dat5 <- dat3 %>%
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_Pet28_controls.pdf"), width = 20, height = 14)
...
a <- dat5 %>%
  ...
  dplyr::slice(0:40) %>% # CHANGED TO 40 because weird bump at 45
  dplyr::ungroup() %>%
