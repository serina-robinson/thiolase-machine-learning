...
fildir <- list.files(paste0("data/", date, "/plate_templates/"), full.names = T) #Plate Set up SynBio Paper heptanoate ClPh.xlsx"
...
temp[!grepl("Pet28_", temp)] <- gsub("ClPh_", "", temp[!grepl("Pet28_", temp)])
temp

## Need to apply to both plates
#### Apply normalize_all to the complete file names
normalize_all_ClPh <- function(x) { 
  x <- tmafils
  tma1 <- read_excel(x) %>%
    janitor::clean_names() %>%
  ...
  # Select only the template files with ClPh, pNP or Pet28 in the name
  ...
  pet28 <- dat0 %>%
    dplyr::filter(grepl("ClPh_Pet28", variable)) %>% # TODO change this to ClPh_pet28
    group_by(time) %>%
...
tmafils
res <- normalize_all_ClPh(tmafils)
head(res)
