...
temp <- read_excel(paste0("data/", date, "/plate_templates/Plate Set up SynBio Paper heptanoate biotin.xlsx"), skip = 4) %>%
  janitor::clean_names() %>%
...
resbind <- res %>%
  dplyr::filter(!grepl("biotin|Nothing_", variable))
...
dat4 <- dat3 %>%
  ...
  dplyr::mutate(winners = case_when(any(mean > 1.5) ~ variable)) %>%
