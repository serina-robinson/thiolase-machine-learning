...
dat4 <- dat3 %>%
  ...
  dplyr::mutate(winners = case_when(any(mean > -2) ~ variable)) %>%
...
a <- dat5 %>%
  ...
  # dplyr::filter(variable %in% winners) %>%
...
orgs <- unique(a$winners)[unique(a$winners) != " inactive"]
orgs
...
levels(as.factor(merg_all$winners)))
pal2[as.numeric(levels(as.factor(merg_all$winners)))]
...
pl <- ggplot(merg_all,  aes(x = minutes, y = mean, color = winners)) + 
  ...
  geom_abline(slope = unique(merg_all$max_slope), intercept = unique(merg_all$intercept), color = "gray") +#pal2[1:length(unique(merg_all$winners))]) +
  labs(y = "nmol pNP", x = "Time (minutes)") +
  theme(legend.title=element_blank(), axis.line=element_line(color="black"),
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
...
pdf(paste0("output/", date, "/", date, "_", cmpnd, "_JGI_genes_linear_slopes_normalized_final.pdf"), width = 10, height = 8)
pl <- ggplot(merg_all_final,  aes(x = minutes, y = mean, color = winners)) + 
  geom_point(alpha = ifelse(merg_all$winners == " inactive", 0.2, 1)) +
  geom_abline(slope = unique(merg_all_final$max_slope), intercept = unique(merg_all_final$intercept), color = "gray") +#color = pal2[1:length(unique(merg_all_final$winners))]) +
  labs(y = "nmol pNP", x = "Time (minutes)") +
  theme(legend.title=element_blank(), axis.line=element_line(color="black"),
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank(),
