...
# Normalize all
rep1 <- fread(tmafils[1], fill = T, header = F, skip = 548, nrows = 548, data.table = F) # specific for heptynoate
head(rep1)
tail(rep1)
res <- lapply(list(rep1), normalize_all_txt)
...
dat4 <- dat3 %>%
  dplyr::filter(grepl(max(time), time)) %>%
  dplyr::mutate(winners = case_when(mean > 2 ~ variable)) %>%
...
write_csv(slope_final, paste0("output/", date, "/", date, "_", cmpnd, "_biorep2_all_data_calculated_slopes.csv"))
