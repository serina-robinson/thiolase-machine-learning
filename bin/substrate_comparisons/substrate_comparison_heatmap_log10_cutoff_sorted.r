...
#  dplyr::filter(max_slope >= 0.1) %>%
 dplyr::mutate(max_slope = max_slope * 10 * 60) %>%
...
maprdat_wide[is.na(maprdat_wide)] <- 1
...
pdf("output/substrate_comparisons/substrate_comparison_heatmap_unclustered_C6_only_log10_scal_per_hr_no_cutoff.pdf", width = 5, height = 10)
pheatmap(
  cluster_cols = T,
  # cluster_rows = T,
  border_color = NA,
  ...
  #breaks = mat_breaks,
  color = pal2,
  #color =  colorRampPalette(brewer.pal(8,"YlOrRd"))(80),
