...
# sub_test <- read_excel("data/sigma_carboxylic_acid_scrape/20191029_substrates_tested_ordered.xlsx") %>%
#   dplyr::pull(1)
# sub_test
...
# head(key)
# key$cmpnd <- gsub("cids\\.", "", key$cmpnd)
# write_csv(key, "data/substrates_tested_CID_cmpnd_name_key.csv")
# 
# write_delim(data.frame(final_df$cid), "data/substrates_tested_CA_cids_output.txt", delim = "\t", col_names = F)

# Combine everything
# big_key <- read_csv("data/CID_cmpnd_name_key.csv")
# small_key <- read_csv("data/substrates_tested_CID_cmpnd_name_key.csv")
# merg_key <- big_key %>%
#   bind_rows(small_key)
# write_csv(merg_key, "data/1127_merged_CID_cmpnd_name_key.csv")
# cids_to_pull <- merg_key$cid
# write_delim(data.frame(cids_to_pull), "data/1127_merged_CA_cids_output.txt", delim = "\t", col_names = F)

final_df <- read_delim("data/1127_merged_CA_cids_output.txt", delim = "\t", col_names = F)

sdfset <- read.SDFset("data/merged_sigma_CAs.sdf")
...
# Identity
status <- read_excel("data/sigma_carboxylic_acid_scrape/20191029_substrates_tested_ordered.xlsx")
  
key <- read_csv("data/1127_merged_CID_cmpnd_name_key.csv")
...
#dplyr::mutate(fixnams = case_when(cid %in% dups$cid ~ dups$cmpnd))

apset@ID <- newnamvec$cmpnd
apset@ID[grep("dodecan", apset@ID)]
...
pdf(paste0("output/", numseqs,"_PCoA_labeled_CA_Sigma.pdf"), width = 15, height = 15)
