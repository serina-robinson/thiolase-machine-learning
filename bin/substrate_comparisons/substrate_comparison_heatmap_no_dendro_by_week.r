...
ll <- ll0[!grepl("BocPhe|C6|furf|rep1|rep2|scratch", ll0)]
hept_rep2 <- ll0[grepl("heptynoate_biorep2", ll0)]
ll0[grepl("heptynoate_biorep2", ll0)]

llc <- c(ll, hept_rep2)

rawdat <- tibble(filename = llc) %>%
  ...
  dplyr::mutate(cmpnd = case_when(grepl("C6", filename) ~ "hexanoate",
                                  grepl("heptynoate_biorep2", filename) ~ "heptynoate rep 2",
                                  grepl("2019-10-25_heptynoate", filename) ~ "heptynoate rep 1",
...
pdf("output/substrate_comparisons/substrate_comparison_heatmap_unclustered_by_week.pdf", width = 6, height = 9)
pheatmap(
  cluster_cols = F,
  cluster_rows = F,
  border_color = NA,
...
pdf("output/substrate_comparisons/substrate_comparison_heatmap_unclustered_by_week_transposed.pdf", width = 12, height = 6)
pheatmap(
  cluster_cols = F,
  cluster_rows = F,
  border_color = NA,
